...
_COUNTER_PREFIX = "claude-tool-count-"
_DEFAULT_THRESHOLD = 50
_MAX_COUNTER_AGE_HOURS = 24


def _find_latest_counter(tmp_dir: str | None = None) -> dict | None:
    """Find the most recent tool-call counter file in the temp directory.

    Returns a dict with 'path', 'count', 'age_seconds', 'session_id'
    or None if no recent counter exists.
    """
    import tempfile
    import time

    search_dir = tmp_dir or tempfile.gettempdir()
    try:
    ...
    for name in entries:
        ...
        if age_seconds > _MAX_COUNTER_AGE_HOURS * 3600:
            continue
        ...
        if best is None or stat.st_mtime > best["mtime"]:
            ...
            best = {
                ...
                "mtime": stat.st_mtime,
                "session_id": name[len(_COUNTER_PREFIX):],
            }
...
def _count_compactions() -> int:
    """Count compaction events from the compaction log."""
    log_path = os.path.join(
        str(os.path.expanduser("~")), ".claude", "sessions", "compaction-log.txt"
    )
    try:
        with open(log_path) as f:
            return sum(1 for line in f if "compaction triggered" in line.lower())
    except OSError:
        return 0


def _compact_suggestion(count: int, threshold: int) -> str:
    if count < threshold:
        return "Session is fine — no need to compact yet."
    if count < threshold + 25:
        return "Approaching threshold — consider compacting if switching phases."
    return "Well past threshold — compact if context feels stale."


@app.command(name="compact-status")
def compact_status(
    ctx: typer.Context,
    json_output: bool = typer.Option(False, "--json", help="Output as JSON."),
) -> None:
    """Show current Claude Code session compaction health.

    Reads the tool-call counter and compaction log to report whether
    it is a good time to run /compact.
    """
    threshold = _DEFAULT_THRESHOLD
    env_thresh = os.environ.get("COMPACT_THRESHOLD")
    if env_thresh and env_thresh.isdigit():
        threshold = int(env_thresh)

    counter = _find_latest_counter()
    compactions = _count_compactions()
    ...
    if counter is None:
        if json_output:
            ...
            console.print("[dim]No active Claude Code session found.[/dim]")
            if compactions > 0:
                console.print(f"[dim]Total compactions (all sessions): {compactions}[/dim]")
        return

    count = counter["count"]
    age_minutes = int(counter["age_seconds"] / 60)
    suggestion = _compact_suggestion(count, threshold)
    ...
    console.print(f"\n[bold]Session:[/bold] {counter['session_id']}")
    console.print(f"  Tool calls:  {count} / {threshold} threshold")
    console.print(f"  Session age: {age_minutes}m")
    console.print(f"  Compactions: {compactions}")
