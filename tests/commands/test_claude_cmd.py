...
class TestFindLatestCounter:
    """Unit tests for _find_latest_counter."""

    def test_no_counter_files(self, tmp_path):
        assert _find_latest_counter(str(tmp_path)) is None

    def test_finds_counter_file(self, tmp_path):
        counter = tmp_path / "claude-tool-count-abc123"
        counter.write_text("42")

        result = _find_latest_counter(str(tmp_path))
        assert result is not None
        assert result["count"] == 42
        assert result["session_id"] == "abc123"
        assert result["age_seconds"] >= 0

    def test_picks_newest_counter(self, tmp_path):
        import time

        old = tmp_path / "claude-tool-count-old"
        old.write_text("10")
        time.sleep(0.05)

        new = tmp_path / "claude-tool-count-new"
        new.write_text("99")
    ...
    def test_ignores_non_counter_files(self, tmp_path):
        (tmp_path / "other-file.txt").write_text("hello")
        (tmp_path / "claude-tool-count-sess1").write_text("5")

        result = _find_latest_counter(str(tmp_path))
        assert result is not None
        assert result["session_id"] == "sess1"

    def test_handles_corrupted_counter(self, tmp_path):
        counter = tmp_path / "claude-tool-count-bad"
    ...
    def test_handles_empty_counter(self, tmp_path):
        counter = tmp_path / "claude-tool-count-empty"
        counter.write_text("")
    ...
    def test_nonexistent_dir(self):
        assert _find_latest_counter("/nonexistent/path/12345") is None


class TestCountCompactions:
    """Unit tests for _count_compactions."""

    def test_counts_compaction_lines(self, tmp_path):
        log_path = tmp_path / ".claude" / "sessions" / "compaction-log.txt"
        log_path.parent.mkdir(parents=True)
        log_path.write_text(
            "[2026-03-19 10:00:00] Context compaction triggered\n"
            "[2026-03-19 11:00:00] Context compaction triggered\n"
            "[2026-03-19 12:00:00] Something else\n"
        )

        with patch("os.path.expanduser", return_value=str(tmp_path)):
            assert _count_compactions() == 2

    def test_missing_log_returns_zero(self, tmp_path):
        with patch("os.path.expanduser", return_value=str(tmp_path)):
            assert _count_compactions() == 0


class TestCompactSuggestion:
    """Unit tests for _compact_suggestion."""

    def test_below_threshold(self):
        msg = _compact_suggestion(20, 50)
        assert "fine" in msg.lower() or "no need" in msg.lower()

    def test_at_threshold(self):
        msg = _compact_suggestion(50, 50)
        assert "consider" in msg.lower() or "switching" in msg.lower()

    def test_well_past_threshold(self):
        msg = _compact_suggestion(100, 50)
        assert "stale" in msg.lower() or "past" in msg.lower()


class TestCompactStatusCommand:
    """CLI tests for tai claude compact-status."""

    def test_no_active_session(self, tmp_path):
        with patch(
            "tai.commands.claude._find_latest_counter", return_value=None
        ), patch("tai.commands.claude._count_compactions", return_value=0):
            result = runner.invoke(app, ["compact-status"])

        assert result.exit_code == 0
        assert "No active" in result.output

    def test_no_active_session_json(self, tmp_path):
        with patch(
            "tai.commands.claude._find_latest_counter", return_value=None
        ), patch("tai.commands.claude._count_compactions", return_value=0):
            result = runner.invoke(app, ["compact-status", "--json"])
    ...
    def test_active_session(self):
        counter = {
            "path": "/tmp/claude-tool-count-test",
            "count": 30,
            "age_seconds": 600,
            "session_id": "test",
        }
        with patch(
            "tai.commands.claude._find_latest_counter", return_value=counter
        ), patch("tai.commands.claude._count_compactions", return_value=1):
            result = runner.invoke(app, ["compact-status"])
    ...
    def test_active_session_json(self):
        counter = {
            "path": "/tmp/claude-tool-count-sess1",
            "count": 60,
            "age_seconds": 1200,
            "session_id": "sess1",
        }
        with patch(
            "tai.commands.claude._find_latest_counter", return_value=counter
        ), patch("tai.commands.claude._count_compactions", return_value=2):
            result = runner.invoke(app, ["compact-status", "--json"])

        assert result.exit_code == 0
        data = json.loads(result.output)
        assert data["active_session"] is True
        assert data["tool_calls"] == 60
