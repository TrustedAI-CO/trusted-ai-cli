...
description: |
  ...
  Invoke with: /execute-solo [path-to-plan]
allowed-tools:
  - Bash
  - Read
  - Write
...
```
/tai-plan-eng ──► [plan file] ──► /execute-solo ──► [commits + summary] ──► /tai-ship
```
...
# Execution Summary
Generated by /execute-solo on {date}
Branch: {branch}
Plan: {plan_path}
...
If PARTIAL_SUCCESS: "Some tasks failed. Review the failures above. You can fix manually and /tai-ship, or re-run /execute-solo to retry failed tasks."
