...
4. **TODOS cross-reference:** Read `docs/plan/todos.md` if it exists. Are any deferred items blocking this plan? Can any deferred items be bundled into this PR without expanding scope? Does this plan create new work that should be captured as a TODO?
...
mkdir -p "$_DOCS_DIR/design" "$_DOCS_DIR/specs" "$_DOCS_DIR/plan"
```

Generate these documents (create or update if they exist):

#### 1. Architecture Design (`docs/design/system.md`)
...
type: design
parent: intent
children: [spec-{module1}, spec-{module2}]
related: []
---
...
## Architecture
{ASCII diagram and component descriptions from architecture review}
...
If `docs/intent.md` exists, add `design-system` to its `children` list.

#### 2. Module Specs (`docs/specs/{module}.md`)
...
- **Acceptance:** {criteria}
- **REQs covered:** REQ-{MOD}-NNN, REQ-{MOD}-NNN
...
## Exit Criteria
- [ ] All tasks in docs/plan/tasks.md completed
- [ ] All REQs in docs/specs/ have traceability entries
- [ ] Tests pass
...
- docs/plan/todos.md updates: ___ items proposed to user
...
echo "{\"skill\":\"plan-eng\",\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"status\":\"STATUS\",\"commit_hash\":\"$_COMMIT\",\"unresolved\":N,\"critical_gaps\":N,\"mode\":\"MODE\"}" >> "$_REPO_ROOT/.tai/state/${_BRANCH_SAFE}-reviews.jsonl"
```
