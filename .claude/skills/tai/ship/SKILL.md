...
There is **no post-ship docs-update sweep**. Derived docs are maintained LIVE by
`/tai-execute`; Step 5.75 (check 7) already blocks if they're out of sync with the shipped
diff. A stale derived doc is a gate failure to fix before shipping, never patched afterward.
`/docs-update` survives only as an on-demand refresh tool, never an auto-run ship step.
