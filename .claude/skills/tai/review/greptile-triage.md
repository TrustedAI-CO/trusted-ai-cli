...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/tai/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.tai-skills/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/tai/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.tai-skills/projects/$REMOTE_SLUG"
mkdir -p ~/.tai
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.tai-skills/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.tai-skills/greptile-history.md` (global aggregate)
