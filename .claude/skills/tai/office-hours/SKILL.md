...
version: 1.0.0
description: |
  [TAI] Office Hours — product thinking before code. Two modes. Startup mode: six
  forcing questions that expose demand reality, status quo, desperate specificity,
  narrowest wedge, observation, and future-fit. Builder mode: design thinking
  brainstorming for side projects, hackathons, learning, and open source.
  Saves a design doc. Never writes code.
  ...
  Use before /plan-ceo or /plan-eng.
allowed-tools:
  - Bash
  - Read
  - Grep
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.tai-skills/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.tai-skills/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.tai-skills/projects/`.
...
Count the signals. You'll use this count in Phase 6.
...
```bash
PRIOR=$(ls -t ~/.tai-skills/projects/$SLUG/*-$_BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.tai-skills/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
- **`/plan-ceo`** for ambitious features (EXPANSION mode) — rethink the problem, find the 10-star product
...
The design doc at `~/.tai-skills/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
