...
`~/.claude/skills/tai/gen-content` as the default install location.
...
If this skill hits a `tai` bug/limitation, file a field report per the **tai Field Report**
convention in `docs-conventions.md` (single source of truth, ADR 0005).
...
```bash
cat "${SKILL_DIR}/references/voice-profile-guide.md" 2>/dev/null || cat ~/.claude/skills/tai/gen-content/references/voice-profile-guide.md 2>/dev/null || echo "GUIDE_NOT_FOUND"
```
...
```bash
cat docs/changelog.md 2>/dev/null || echo "No CHANGELOG found"
```
...
```bash
cat "${SKILL_DIR}/references/visualization-guide.md" 2>/dev/null || cat ~/.claude/skills/tai/gen-content/references/visualization-guide.md 2>/dev/null || echo "VIZ_GUIDE_NOT_FOUND"
```
...
```bash
cat "${SKILL_DIR}/references/slop-patterns.md" 2>/dev/null || cat ~/.claude/skills/tai/gen-content/references/slop-patterns.md 2>/dev/null || echo "SLOP_FILE_NOT_FOUND"
```
