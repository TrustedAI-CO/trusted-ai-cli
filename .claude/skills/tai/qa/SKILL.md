...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/tai/browse/dist/browse" ] && B="$_ROOT/.claude/skills/tai/browse/dist/browse"
[ -z "$B" ] && B=~/.claude/skills/tai/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "tai browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
1. **Project-scoped test plans:** Check `~/.tai-skills/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   _SLUG=$(basename "$(git remote get-url origin 2>/dev/null)" .git 2>/dev/null || echo "project")
   ls -t ~/.tai-skills/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
2. **Conversation context:** Check if a prior `/plan-eng` or `/plan-ceo` produced test plan output in this conversation
