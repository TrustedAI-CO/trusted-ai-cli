...
version: 1.0.0
description: |
  [TAI] Launch a parallel team of engineers to implement a phase from the plan.
...
║  Engineer: /tai-plan-eng → /execute-solo → /tai-ship          ║
║  Reviewer: /tai-review    QA: /qa                   ║
╚═══════════════════════════════════════════════════════════╝
```
...
1. **Spawn QA agent within the team** (never run QA inline — always delegate):
   ...
   Agent(
     ...
     prompt: "You are the QA tester for phase {N}.{M} on branch {branch}.
       ...
       2. Run /qa to systematically test the changes.
          /qa produces a structured report with health score,
...
### 2. Execute: run /execute-solo
Run `/execute-solo` on the plan produced by step 1. This breaks the plan
into tasks and dispatches each coding task to an external AI CLI (Codex or
Gemini) via `tai agent run`. The routing is automatic — /execute-solo picks
...
If a backend fails, /execute-solo retries with the other backend automatically.
...
3. For larger fixes: dispatch to external backend via `tai agent run`:
   ...
   ```
   Use the same routing rules as /execute-solo (backend vs frontend files).
